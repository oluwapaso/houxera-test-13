...
const FloatingInput = ({ name, label, placeholder, handleChange, handleBlur, type, required = false, value, disabled_field, styles, ...rest }:
    {
    ...
    return (
        <div className={`w-full bg-white border-2 border-gray-300 px-5 py-3 flex flex-col rounded-md 
            ${disabled_field && "!opacity-45 !cursor-not-allowed"} ${styles}`} id='parent_id'>
            <div className='w-full flex items-center text-base font-medium'>
                <span className=' line-clamp-1'>{label}</span> {required && <FaAsterisk className='text-red-600 ml-1' size={12} />}
            </div>
