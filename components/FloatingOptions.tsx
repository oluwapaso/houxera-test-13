...
const FloatingOptions = ({ name, label, handleSelectChange, required = false, options, disabled_field, value, px, py }:
    {
        name: string, label: string, handleSelectChange: (e: React.ChangeEvent<HTMLSelectElement>) => void,
        required?: boolean, options: any[], disabled_field?: boolean, value?: string, px?: string, py?: string
    }) => {

    return (
        <div className={`w-full bg-white border-2 border-gray-300 flex flex-col rounded-md 
            ${disabled_field && "!opacity-45 !cursor-not-allowed"} ${px ? px : "px-5"} ${py ? py : "py-3"}`}>
